...
async function fetchProduct(id, img, title, price, total){
    ...
    price.textContent = (product['price']+",00 €");
     ...
     console.log(typeof(productInt));
     console.log(total);
     console.log(typeof(total));
     return total += productInt
...
for(productCart of localStorageCartJS){
   ...
   let response = fetchProduct(productId, itemImg, descriptionTitleHeader, descriptionPriceParagraph, totalPrice).then((res,err)=> {
   ...
   console.log(response);
    ...
    totalQuantity += new Number(productCart.quantity);
